...
EPOCHS           = 600
BATCH_SIZE       = 2
# Eval runs on rank 0 only, without grad, but the GATv2 attention builds one
# large per-edge tensor whose size scales with the batch. On 11 GiB cards the
# training memory (Adam states + compile pools) is still resident during eval,
# so a smaller eval batch keeps that single allocation from OOM-ing.
EVAL_BATCH_SIZE  = 1
GRAD_CLIP        = 1.0
# 2080 Ti (~11 GiB): per-step activation peak scales with the largest graph in
# a batch, and at batch_size 4 that peak reached the card limit on heavy batches
# (intermittent OOM in backward). batch_size 2 × grad_accum 8 halves the peak
# while keeping the effective batch identical (2 × 8 × 7 ranks = 112).
GRAD_ACCUM_STEPS = 8
...
#      exercises — become the VALIDATION set; the train cities (large volume,
#      z∈[0,1]) become the TRAIN set. No separate test set: validation is the
#      final evaluation (whole cities never trained on → geometric generalisation).
#
#   2. Fallback by exercise count (file absent): the N_EVAL_CITIES cities with
#      the fewest graphs are taken as validation, the rest as train. Robust
#      because eval cities are generated with clearly fewer exercises.
#
