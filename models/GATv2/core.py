...
class GATv2(torch.nn.Module):
    """Multi-layer GATv2Conv with additive drone-signature fusion.

    GATv2 (Brody et al., 2022) fixes the 'static attention' problem of GATv1
    by applying the non-linearity *before* computing attention scores:

        e_ij = a^T · LeakyReLU( W · [h_i || h_j] )   ← GATv2
        e_ij = a^T · LeakyReLU( W·h_i + W·h_j )       ← GATv1  (factorisable)

    Each layer averages `num_heads` attention heads (concat=False) to keep
    `hidden_channels` constant throughout, with residual connections +
    LayerNorm.

    `grad_checkpoint=True` wraps each GATv2Conv block in activation
    ...
    def __init__(
        ...
        # ── GATv2 layers ─────────────────────────────────────────────────────
        # GATv2Conv expose `edge_dim` et `share_weights` en plus de GATConv.
        #   • concat=False  → sortie de taille hidden_channels (moyenne des têtes)
        #   • dropout       → dropout sur les coefficients d'attention
        self.convs = torch.nn.ModuleList([
            GATv2Conv(
                ...
                share_weights=False,   # True économise des params, False = plus expressif
            )
    ...
    # ── Bloc élémentaire (checkpointable) ────────────────────────────────────
    def _layer_block(
        self,
        conv:       GATv2Conv,
        ...
        """Conv → Norm → ReLU → Dropout  (résidu ajouté dans forward)."""
        return F.dropout(
            F.relu(norm(conv(h, edge_index))),
            p=self.dropout,
    ...
    def forward(
        self,
        x:          torch.Tensor,   # [N, num_node_features]
        edge_index: torch.Tensor,   # [2, E]
        drone_feat: torch.Tensor,   # [B, num_drone_features]  (B = nb de graphes)
        batch:      torch.Tensor,   # [N]  mapping nœud → graphe
        ...
        for conv, norm in zip(self.convs, self.norms):
            if use_ckpt:
                # use_reentrant=False : requis pour DDP et torch.compile
                residual = torch.utils.checkpoint.checkpoint(
                    self._layer_block, conv, norm, h, edge_index,
                    use_reentrant=False,
                )
            else:
                residual = self._layer_block(conv, norm, h, edge_index)

            h = h + residual   # connexion résiduelle
