...
# Rôles éval/train par ville : écrit par generate_dataset.py à côté des PLY, on
# le recopie dans processed/ pour que le split (dataio) sache quelles villes sont
# l'évaluation. Aucun exercice n'est jeté ici : le contrôle du nombre d'exercices
# se fait entièrement en amont, dans generate_dataset.py.
ROLES_FILE_NAME = "city_roles.json"    # source, à la racine de generated/
SPLIT_FILE_NAME = "city_split.json"    # copie, dans processed/ (lue par dataio)
...
from models.GATv2.config import (
    META, NUM_FEATURES, FEAT_KEYS, FNAME_RE,
)
...
def _bootstrap_manifest_from_shards(shard_dir: Path) -> set[str]:
    ...
    for s in tqdm(shards, desc="scan-shards", unit="shard"):
        ...
        for g in graphs:
            ...
            # Convention (cf. generate_dataset.run_one) : les floats sont écrits
            # avec 4 décimales → on DOIT reproduire ce format, sinon le nom
            # reconstruit ne matche jamais le PLY réel et tout serait retraité
            # (→ shards dupliqués).
            fname = f"NoiseMap_{map_name}_{x:.4f}_{y:.4f}_{z:.4f}_{drone_id}.ply"
...
# ─────────────────────────────────────────────────────────────────────────────
# Propagation des rôles de villes (éval/train → split dataio)
# ─────────────────────────────────────────────────────────────────────────────

def _propagate_city_roles(generated_dir: Path, out_dir: Path) -> None:
    """Recopie `generated_dir/city_roles.json` (écrit par generate_dataset.py)
    vers `out_dir/city_split.json`, lu par le split de dataio. Aucun exercice
    n'est filtré ici : on ne fait que transporter l'info éval/train dans le
    dossier processed/. Silencieux si le fichier de rôles est absent."""
    src = generated_dir / ROLES_FILE_NAME
    dst = out_dir / SPLIT_FILE_NAME
    if not src.exists():
        log.info(
            f"Aucun {ROLES_FILE_NAME} dans {generated_dir} : le split retombera "
            f"sur le repli par compteur d'exercices (dataio)."
        )
        return
    out_dir.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(src, dst)
    try:
        info = json.loads(dst.read_text())
        log.info(
            f"Rôles villes propagés → {dst} "
            f"({len(info.get('eval_cities', []))} éval, "
            f"{len(info.get('train_cities', []))} train)"
        )
    except Exception:
        log.info(f"Rôles villes propagés → {dst}")
...
def build_dataset(
    ...
    # ── Rôles éval/train : on propage city_roles.json → processed/city_split.json
    # (aucun exercice n'est jeté ici ; tout est déjà généré au bon volume).
    _propagate_city_roles(generated_dir, shard_dir.parent)
