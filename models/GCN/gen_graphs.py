...
Node-level 7-class classification.
  ...
  x (5 features, ALL source-relative or scale-invariant):
...
N_BANDS        = 24
NUM_FEATURES   = 5   # voir header du fichier
...
# Node features : (mean, std) pour chaque des 5 features
# Index : [log_dist, cos_ns, rel_x, rel_y, rel_z]
NODE_STATS = {
    # log10(dist+1) : dist typique 1-300 m → log10 ∈ [0, 2.48]
    "log_dist":  (1.2,  0.5),
    # cos(normal, dir_src) ∈ [-1, 1], moyenne ~0
    "cos_ns":    (0.0,  0.57),
    # direction relative normalisée : moyenne ~0, std ~0.577
    "rel_dir":   (0.0,  0.577),
}
...
DRONE_NORM = {
    "n_blades": (4.0, 6.0),
    "rpm":      (800.0, 7000.0),
    "lp_ref":   (16.0, 75.0),   # min/max sur tous les drones
    "slope":    (-1.3e-4, 4.5e-2),
}
...
def _normalize_node_features(feats: np.ndarray) -> np.ndarray:
    ...
    means = np.array([
        ...
        s["rel_dir"][0],   s["rel_dir"][0],   s["rel_dir"][0],
    ], dtype=np.float32)

    stds = np.array([
        s["log_dist"][1],  s["cos_ns"][1],
        s["rel_dir"][1],   s["rel_dir"][1],   s["rel_dir"][1],
    ], dtype=np.float32)
...
def _build_one(args_tuple: tuple[str, bool]):
    ...
    feats, drone_feat, _ = build_node_features(
        verts, faces, src, drone, normalize=normalize
    )
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for i, fut in enumerate(as_completed(futures), 1):
            ...
            if out is None:
                ...
                # _face_adjacency already returns unique undirected pairs.
